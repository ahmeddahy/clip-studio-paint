...
def ProbabilityDensityFunction(x, mean, variance):
    ...
    for i in range(0, len(x), 1):
        pdf = pdf * (1 / math.sqrt(2 * math.pi * variance[i])) * math.exp(
            (-1 * math.pow(x[i] - mean[i], 2)) / (2 * variance[i]))
    return pdf
    ...
    for sample in range(0, NumberOFSamples, 1):
        ...
        for cluster in range(0, gmm.n_components, 1):
            variance = gmm.covariances_[
                cluster].diagonal()  # extract the variance of each class from its covariance matrix
